...
`content/documents/paperpool.md` 使用纯 Markdown 层级，不再使用大表格。固定结构：`# Paper Pool` → 顶部精简维护约定 → `## 已精读论文` / `## 待精读论文`。已精读论文按 `### YYYY-MM-DD` 分组；每篇论文以标题作为一级无序列表项，论文内部字段必须拆成缩进的二级无序列表，禁止把所有字段压缩到同一行。同一天允许多篇。每篇固定保留：`简称`、`作者`、`Venue / 年份`、`DOI / 原文`、`主题`、`一句话价值`；不保留可由所在分区推断的“状态”等冗余字段。简称优先使用论文/方法/benchmark 自己明确使用的名称（如 RepoMirage、AACR-Bench、MCR-Bench）；没有明确简称时使用稳定、简短、可识别的工作简称，后续保持一致。
